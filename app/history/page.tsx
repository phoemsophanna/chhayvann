    ...
    return (
        <div>
            <Layout headerStyle={1} footerStyle={3} breadcrumbTitle="Our History" breadcrumbImage={banner?.image}>
                <section className={`history-style1 history-content ${history?.length > 0 ? "pb-0" : ""}`}>
                    <div className="container">
                        <div dangerouslySetInnerHTML={{__html: sanitizeHtml(settings?.history_description_eng)}}></div>
                    </div>
                </section>
                {
                    history?.length > 0 ? (
